...
function StoreDetail({ store, close }: { store: typeof stores[number]; close: () => void }) {
  return (
    <div className="detail-backdrop" onClick={close}>
      <aside className="store-detail" onClick={(event) => event.stopPropagation()} aria-label={`${store.name} 상세정보`}>
        <div className="detail-top">
          <div className="detail-store-icon">{store.name[0]}</div>
          <div>
            <span className="panel-label">STORE PROFILE</span>
            <h2>{store.name}</h2>
            <p>{store.address}</p>
          </div>
          <button className="detail-close" type="button" onClick={close} aria-label="닫기">×</button>
        </div>
        ...
        <div className="detail-status">
          ...
          <span className="status-pill success">● 정상 운영</span>
        </div>

        <section className="detail-section">
          <h3>가맹점 기본 정보</h3>
          <dl className="detail-list">
            <div><dt>점주명</dt><dd>{store.owner}</dd></div>
            <div><dt>연락처</dt><dd>{store.phone}</dd></div>
            <div><dt>오픈일</dt><dd>{store.opened}</dd></div>
            <div><dt>계약 만료일</dt><dd>{store.contract}</dd></div>
          </dl>
        </section>
        ...
        <section className="detail-section">
          ...
          <div className="detail-metrics">
            <div><span>매출</span><strong>{store.sales}</strong></div>
            <div><span>주문</span><strong>{store.orders}</strong></div>
            <div><span>위생 점수</span><strong>{store.score}점</strong></div>
            <div><span>최근 점검</span><strong>{store.lastCheck}</strong></div>
          </div>
        </section>
        ...
        <section className="detail-section">
          <div className="detail-title-row">
            <h3>최근 특이사항</h3>
            <button className="link-button" type="button">전체 이력</button>
          </div>

          <div className="detail-notice">
            <span className={`alert-dot ${store.risk === '높음' ? 'danger' : 'info'}`}></span>
            <div>
              <strong>{store.risk === '높음' ? '위생 개선 확인이 필요합니다' : '특이사항 없이 정상 운영 중입니다'}</strong>
              <p>
                {store.risk === '높음'
                  ? '조리대 점검 사진에서 청결도 저하 가능성이 감지되었습니다.'
                  : '최근 점검 및 운영 지표가 기준 범위 내에 있습니다.'}
              </p>
            </div>
          </div>
        </section>
        ...
        <div className="detail-actions">
          <button className="outline-button" type="button">점주에게 연락</button>
          <button className="primary-action" type="button">운영 리포트 보기</button>
        </div>
      </aside>
    </div>
  )
